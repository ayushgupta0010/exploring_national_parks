...
function ParkInfoComponent() {
  ...
  useEffect(() => {
    fetch(
      `https://developer.nps.gov/api/v1/events?parkCode=${parkCode}&api_key=0ilOFP8jTC2LMrwXFTullFqvHyVhBh9aHVW3OWEb`,
    )
      .then((res) => res.json())
      .then((data) => setEvents(data.data));
  }, [parkCode]);

  if (parkJSON.length > 1) {
    //list all the parks
    return (
      <div className="top-padding-info">
          ...
          <div className="parks">
            {parkJSON?.map((park) => (
              <div key={park.id} className="post-card">
                <a
                  className="park-info-link"
                  href={"ParkInfo?parkCode=" + park.parkCode}
                >
                  <div>
                    ...
                    <img
                      src={park.images.length !== 0 ? park.images[0].url : ""}
                      alt=""
                      width="100"
                      height="300"
                    />
        ...
        <a href={"./ParkInfo?page=" + pageDown}>
          <button className="park-info-button">Previous Page</button>
        </a>
        <a href={"./ParkInfo?page=" + pageUp}>
          <button className="park-info-button">Next Page</button>
        </a>
      </div>
    );
  } else {
              ...
              <center>
                <div className="box-1">
                  ...
                  <div className="hours">
                    <h1>Events:</h1>
                    {events.map((event) => (
                      <div className="events" key={event.eventid}>
                        <p>{event.title}</p>
                        <p>Start: {event.recurrencedatestart}</p>
                        <p>End: {event.recurrencedateend}</p>
                      </div>
                    ))}
                  </div>
                  <div className="hours">
                    ...
                    <ul className="hours-list">
                      ...
                      <li>
                        Sunday: {park.operatingHours[0].standardHours.sunday}
                      </li>
                    </ul>
                    <br></br>
                    ...
                    <br></br>
                    <a href="./ParkInfo">
                      <button className="park-info-button">
                        Return To Parks
                      </button>
                    </a>
                    <a href={"./ParkPlan?parkCode=" + park.parkCode}>
                      <button className="park-info-button">Plan A Trip</button>
                    </a>
                  </div>
                </div>
              </center>
